...
void SurfaceTransparencyDrawer::resize(int w, int h)
{
	...
	texture_color1_->setImage2D_simple(w, h, GL_RED, GL_RED, GL_FLOAT);
	texture_color1_->release();

	texture_color2_ = cgogn::make_unique<cgogn::rendering::ogl::Texture>();
	texture_color2_->bind();
	texture_color2_->setImage2D_simple(w, h, GL_RED, GL_RED, GL_FLOAT);
	...
	texture_color4_->setImage2D_simple(w, h, GL_RED, GL_RED, GL_FLOAT);
